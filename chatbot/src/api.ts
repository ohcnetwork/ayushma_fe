...
const request = async (
  ...
  const base_url =
    (localStorage && localStorage.getItem("ayushma-chatbot-api-url")) ||
    "https://ayushma-api.ohc.network/api/";

  let url = external ? endpoint : base_url + endpoint;
  let payload = formdata ? data : JSON.stringify(data);

  const storage =
    isAuth === false ? null : localStorage.getItem("ayushma-chatbot-token");
...
export const API = {
  ...
  chat: {
    ...
    converse: (
      ...
      chat?: Chat,
      onMessage:
        | ((event: ChatConverseStream, chat: Chat) => void)
        | null = null,
