...
export default function ChatMain(props: {
  ...
  const {
    projectID,
    chat,
    streamingMessage: initStreaming,
    isTyping,
    chatMessage,
  } = props;

  const [streamingMessage, setStreamingMessage] =
    React.useState<string>(initStreaming);
...
function ChatBlock(props: { message: Partial<ChatMessage> }) {
  const { message } = props;
  ...
  return (
    ...
    >
      <div
        className={`px-3 text-sm py-2 border rounded-lg ${
          message.messageType === ChatMessageType.AYUSHMA
            ? "bg-white mr-4"
            : "bg-green-700 text-white ml-4"
        }`}
      >
        {message.message}
      </div>
