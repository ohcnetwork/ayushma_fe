...
const RoleButton = (
  ...
  return (
    <button
      {...rest}
      className={twMerge(
        `w-full sm:w-fit flex justify-center gap-1 border border-tertiaryBorderColor px-2 py-1 rounded-full items-center ${color === "green" && "hover:border-green-400"
...
export default function Page() {
  ...
  useEffect(() => {
    userQuery.refetch();
  }, [searchString, isAdmin, isReviewer, isKeyAllowed]);
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 flex-col sm:flex-row">
        ...
        />
        <RoleButton
          onClick={() => setIsReviewer((s) => !s)}
          color="orange"
          text="Reviewer"
          state={isReviewer}
        />
        <RoleButton
          onClick={() => setIsKeyAllowed((s) => !s)}
          color="blue"
          text="Key Allowed"
          state={isKeyAllowed}
        />
      ...
      <div className="relative overflow-x-auto mt-2 sm:rounded-lg">
        ...
        >
          <table className="w-full text-sm text-left mt-4">
            <thead className="text-xs text-tertiaryTextColor uppercase bg-secondary border border-tertiaryBorderColor rounded-lg">
              <tr>
                  ...
                  {usersList.results.map((user: User) => {
                    return (
                      ...
                      >
                        <td
                          scope="row"
                          className="px-6 py-2 font-medium text-tertiaryTextColor"
                        >
                          <div className="flex flex-col">
                            <div className="flex gap-2 items-center">
                              ...
                              {user.allow_key && (
                                <i
                                  className="fa fa-key text-xs text-tertiaryTextColor"
                                  aria-hidden="true"
                                />
                              )}
                            </div>
                        ...
                        <td className="px-6 py-2">
                          <div className="flex gap-2">
                            {!user.is_staff && !user.is_reviewer && (
                              <RoleBubble color="gray" text="User" />
                            )}
                            {user.is_staff && (
                              <RoleBubble color="green" text="Admin" />
                            )}
                            {user.is_reviewer && (
                              <RoleBubble color="orange" text="Reviewer" />
                            )}
                          </div>
                        </td>
